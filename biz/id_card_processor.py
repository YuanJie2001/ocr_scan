...
class IDCardProcessor(ImageABC):
    ...
    def process_result(self, result):
        ...
        # 验证身份证号码
        if (
            "公民身份号码" in result_dict
            and result_dict["公民身份号码"] != "未识别"
            and result_dict["公民身份号码"] != "识别错误"
        ):
    ...
    def process_id_card(
        self, img_path, out_path=None, show_process=False, convert_to_scan=False
    ):
        """
        处理身份证图像并进行OCR识别
        ...
        if ocr_result:
            ...
            for i, item in enumerate(ocr_result):
                ...
                for label in labels:
                    ...
                    if (
                        text == label
                        and label_values[label] == "未识别"
                        and i + 1 < len(ocr_result)
                    ):
